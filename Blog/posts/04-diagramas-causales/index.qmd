...
```{r}
asp_dag <- dagitty('dag{U -> T; U -> Y; T -> A; A -> Y}') 

impliedConditionalIndependencies(asp_dag)

```

Si tengo datos, puedo poner a prueba la correspondencia del **DAG** con lo _observado_. Me parece que no le gstan los `tibble` a esta función
```{r}
localTests(asp_dag, as.data.frame(datos_aspirina), "cis.pillai")
```
Si quiero aprovechar la caraacterización de las variables puuedo hacer cosas como rgaficar el *subgrafo* que corresponde con la puerta trasera
...
library(DT)
datatable(t(tabla_dag), 
          colnames = c("Tipo de variable", "variable"))
